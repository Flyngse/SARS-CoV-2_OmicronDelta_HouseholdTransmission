...
infile "F:\Projekter\FSEID00004942\FrederikPlesnerLyngse\Omnicron\Data_Revision1\basis_samples_&Update..csv" delimiter = ';' MISSOVER DSD lrecl=32767 firstobs=2 ;
...
proc sql;
	...
	left join	data.VariantPCR					V
		on P.pnr = V.pnr
		and P.PrDate = V.PrDate
;quit;
*/
* Merge MIBA, VariantPCR, WGS ;
proc sql;
...
data data.Positive_PCR_Tests_4 ;
	set data.Positive_PCR_Tests_4 ;
		if VariantPCR_selected = . then VariantPCR_selected = 0 ;
		CT_value_exist = 1 ;  
