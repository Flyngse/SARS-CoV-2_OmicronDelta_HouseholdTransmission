...
proc genmod data = data.Analysis_daily plots=none ;
	...
	class 
			...
			time_from_index
			/ param=glm;
	model	ever_test_PCR_1_100 = time_from_index   / noint ;
...
proc genmod data = data.Analysis_daily plots=none ;
	...
	class 
			...
			time_from_index
			/ param=glm;
	model	ever_test_2_PCR_100 = time_from_index   / noint ;
	repeated subject=house_number / type=ind ;	
run;quit;
...
ods graphics / reset=index imagename = "ProbTestPCR" imagefmt=pdf ;
...
proc sgplot data = parms ;
	...
	series
			y= estimate
			x= Days
			/ markers group=group name="estimates" datalabel=estimate  datalabelattrs=(size=16)
			;
		band 
			x = Days
			lower=LowerCL
...
proc genmod data = data.Analysis_daily plots=none ;
	...
	class 
			...
			time_from_index
			/ param=glm;
	model	ever_test_pos_PCR_100 = time_from_index   / noint ;
	repeated subject=house_number / type=ind ;	
run;quit;
...
ods listing gpath = "F:\Projekter\FSEID00004942\FrederikPlesnerLyngse\Omnicron\Output_Revision1\" ;
ods graphics / reset=index imagename = "ProbTestPosPCR" imagefmt=pdf ;
ods graphics on / attrpriority=none border=off ;
Title ;
proc sgplot data = parms ;
	where Days > 0 ;
